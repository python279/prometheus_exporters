...
dataDir = sys.argv[1]
logDir = sys.argv[2]
logger = logging.getLogger('ganglia_parser.py')
hdlr = logging.FileHandler('%s/ganglia_parser.py.log' % logDir)
...
def listXMLSources():
    ...
    work when the ganglia_parser.py script is run on the same host as the ganglia
...
def main():
    ...
    logger.info('Starting ganglia_parser.py')
    try:
    ...
    splitXML(dataDir)
    logger.info('Finished ganglia_parser.py')
